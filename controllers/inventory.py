...
def unit_of_measure():
    return crud(db.unit_of_measure)

@auth.requires(lambda: auth_has_access())
def system():
    return crud(db.system)

@auth.requires(lambda: auth_has_access())
def subsystem():
    return crud(db.subsystem)

@auth.requires(lambda: auth_has_access())
def system_item():
    return crud(db.system_item)
...
def inventory_item():
    ...
    def do_form_success(form):
        ...
        if item_type == 'tire' \
        and db(db.groove_annotation.tire_id == id).count() == 0:
    ...
    oform = ONXFORM(db.inventory_item)
    ...
    if action in['update', 'read']:
        record = db.inventory_item[record_id]
...
@auth.requires(lambda: auth_has_access())
def inventory_item_autocreate():
    count = int(request.args[0])
    InventoryModel.inventory_item_autocreate(count)
    redirect(URL(f='inventory_item'))
    return
        ...
        def _do_get_content(*args):
            if action == 'list':
                ...
                for row in rows:
                    ...
                    if row.owner_table == 'vehicle':
                        row.repr.owner_key = '%(licence_plate)s - %(description)s' % db.vehicle[int(row.owner_key)]
                return rows
...
def components():
    ...
    if owner_table == 'vehicle':
        vehicle = db(db.vehicle.id == int(owner_key)).select().first()
    ...
    items = db((db.component_item.owner_table == owner_table) & (db.component_item.owner_key == owner_key)).select()
    for item in items:
        ONXREPR.row_repr(item, db.component_item)

    controls = []
    if db(db.component_item.id == item_selected).count() > 0:
...
def add_component():
    ...
    avaliable_components = (db.inventory_item.item_type == 'component') & (db.inventory_item.status == 'available')
    db.component_item.component_id.requires = IS_IN_DB(db(avaliable_components), db.inventory_item, db.inventory_item._format)

    form = SQLFORM.factory(
        db.component_item.component_id,
    ...
    if form.process(formname='add_component_form').accepted:
        ...
        defs = table_default_values(db.component_item)
        defs['component_id'] = component_id
        defs['owner_table'] = owner_table
        defs['owner_key'] = owner_key
        defs['owner_link'] = owner_link

        db.component_item.insert(**defs)
        InventoryModel.change_item_status(component_id, 'in_use', owner_table, owner_key, owner_link)
...
def selected_component():
    ...
    item = db((db.component_item.id == item_id) \
        & (db.inventory_item.id == db.component_item.component_id)
        ).select().first()
    ONXREPR.row_repr(item.component_item, db.component_item)
...
def remove_component():
    ...
    item = db(db.component_item.id == item_id).select().first()
    component_id = item.component_id
    db(db.component_item.id == item_id).delete()
    InventoryModel.change_item_status(component_id, 'available', None, None, None)
    response.js = """
        window.location.reload(true);
        """
    return
